...
close all
root = fileparts(pwd);
patchwarp_path = fullfile(root, 'PatchWarp');
addpath(genpath(patchwarp_path))

%% Specifiy source directory and saving directory
% Image data needs to be tiff stack files.
folder_dir = fullfile(root, 'Widefield');
folder_name = '011022_GC33_GCaMP';
file_name = 'right_register_patchwarp';
ops.source_path = fullfile(folder_dir, folder_name, file_name);    % Directory that contains original tiff stack files
ops.save_path = fullfile(folder_dir, folder_name, file_name);    % Directory where motion corrected images will be saved
